...
def read_csv_results(csv_path, batch_size=16, negatives=5250):

    column_names = ['N', 'TH', 'Model', 'Packet type', 'TPS', 'FPS']
    data = pd.read_csv(csv_path, names=column_names)
    group = data.groupby(['N', 'TH', 'Packet type'])
    data = group.sum(numeric_only=True)

    data['TPR'] = data.TPS / (batch_size * group.count().TPS)
    data['FPR'] = data.FPS / \
        ((negatives * batch_size - batch_size) * group.count().FPS)
